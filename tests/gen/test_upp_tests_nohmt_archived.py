...
def test_upp_tests_counts(
    ...
    _share_distribution = SHRDistribution[_share_distribution]
    ...
    if _share_distribution.name != "UNI":
        ...
        # Test firm counts
        if not (
            _pcm_restriction.name == "MNL"
            and (
                (
                    _pcm_distribution.name.startswith("EMPR_")
                    and _price_spec.name.startswith("PRICE_")
                )
                or (
                    _pcm_distribution.name in {"UNI", "BETA"}
                    and _price_spec.name in {"PRICE_RND", "PRICE_POS", "PRICE_NEG"}
                )
            )
        ):
            ...
            # Test firm count counts against firm-count weights
            if not np.allclose(
                _fcounts_counts / _fcounts_counts.sum(),
                market_sample.share_spec.firm_counts_weights,
                atol=1.2 / np.sqrt(SAMPLE_SPEC_AUX["sample_size"]),
            ):
                print(
                    "Firm counts:",
                    _fcounts_counts,
