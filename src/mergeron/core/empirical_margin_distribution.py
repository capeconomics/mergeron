...
firm's financial performance or investments.
"""
...
def margin_data_builder(
    ...
    parameters = np.array([np.nan, np.nan], float)
    for _sk in dmd_keys:
    ...
    parameters = ArrayDouble(parameters[1:, :])

    _obs, _wts = (parameters[:, _f] for _f in range(2))

    # np.average retains the array-type of obs and _wts, which throws
    # the type-checker off; the casting below should be costless
    _gm_avg, num_firms = (
    ...
    # Please mind the syntax variation.
    _bw = improved_sheather_jones(parameters[:, [0]], parameters[:, 1])
    ...
    *, data_archive_path: Path = MGNDATA_ARCHIVE_PATH, data_download_flag: bool = False
    ...
    if data_archive_path.is_file() and not data_download_flag:
        with zipfile.ZipFile(data_archive_path) as _yzp:
            _margin_data: DamodaranMarginData = YAML.load(
